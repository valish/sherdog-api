  ...
  request(url, function(error, response, html) {
    if (!error && response.statusCode == 200) {
      ...
      $('.record .count_history').filter(function() {
        ...
        fighter.record.wins.knockouts = getTotal(winsByKnockout);
        fighter.record.wins.submissions = getTotal(winsBySubmission);
        fighter.record.wins.decisions = getTotal(winsByDecision);
        fighter.record.wins.others = getTotal(winsByOther);
        fighter.record.losses.knockouts = getTotal(lossesByKnockout);
        fighter.record.losses.submissions = getTotal(lossesBySubmission);
        fighter.record.losses.decisions = getTotal(lossesByDecision);
      });
